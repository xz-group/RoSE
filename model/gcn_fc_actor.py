...
class GCN_FC_actor(nn.Module):
	...
	def __init__(self, in_dim_gcn, in_dim_spec, n_latent_var, out_dim):
		"""
			...
			Return:
				None
		"""
		super(GCN_FC_actor, self).__init__()
		...
		# 10 = # of device
		# modify 10 if you change the circuit 
		self.layer_fc_3 = nn.Linear(n_latent_var, out_dim * 10)
	...
	def forward(self, obs_gcn, obs_fc, adj):
		...
		dim_zero = activation_gcn_3.shape[0]
		...
		if shape_dim == 2:
			...
			# 10 = # of device
			output = F.softmax(torch.reshape(self.layer_fc_3(activation_fc_2), (10, 3)), dim=1)
		else:
			activation_gcn_3 = torch.flatten(activation_gcn_3, start_dim=1)
			activation_fc_2 = torch.cat((activation_gcn_3, activation_fc_1), dim=1)
			activation_fc_2 = torch.tanh(self.layer_fc_2(activation_fc_2))
			# 10 = # of device
			output = F.softmax(torch.reshape(self.layer_fc_3(activation_fc_2), (dim_zero, 10, 3)), dim=2)
